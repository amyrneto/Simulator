...
class MyModel:
    ...
    def LoadMeSenpai(self):
        ...
        model = load_model(self.name+'Model.h5')
        '''
        history = self.model.fit([self.X[:2124, :]], [self.NewY[:2124, :]],
    ...
    def predictArray(self, XVal, YVal,X):
        arr = self.model.predict([X[2124:, :]], batch_size=32, verbose=1)
        ...
        while width <=32:
            while dropout <=0.021:
               ...
               # Models[i].LoadMeSenpai()
                Models[i].predict(8+(7-numOfSeparation)*0.125+0.5,4+(7-numOfSeparation)*0.125+0.5)
                iterations=0
                StopTraining=False
                while iterations>0 and StopTraining==False:
                    Models[i].trainMeSenpai(5)
                    StopTraining=Models[i].predict(8+(7-numOfSeparation)*0.125+0.5,4+(7-numOfSeparation)*0.125+0.5)
                    iterations-=1
                #Models[i].LoadMeSenpai()
                #Models[i].plots()
                i += 1
...
for p,j,p1,j1,p2,j2,p3,j3,x,y in zip(Models[use[0]].Result,Models[use[1]].Result,Models[use[2]].Result,Models[use[3]].Result,Models[use[4]].Result,Models[use[5]].Result,Models[use[6]].Result,Models[use[7]].Result,Models[use[0]].AccurateY,Models[use[0]].AccurateYy):
    print((p+j+p1+j1+p2+j2+p3+j3)/8,x,y)
    #print()

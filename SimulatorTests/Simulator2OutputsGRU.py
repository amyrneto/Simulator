...
from keras.layers import GRU,Dense, Dropout, Activation, Flatten, BatchNormalization,Input
...
class MyModel:
    ...
    def __init__(self, X, Y,layerNum,features,NodesMultipliedByLayers,LayerStandardWidth,dropoutPercentage,epochs,Xval,YVal):
        ...
        self.name='0Recurrent'+str(layerNum)+'Layers'+str(LayerStandardWidth)+'Width'+str(dropoutPercentage)+'dropout'+str(Xval)+'Xval'
    ...
    def giveAModelNum1(self):
        x = int(self.X.size / features)
        input1 = Input(shape=(x,features,), name='input1',)
        ...
        oneMoreLayer = GRU(features*3,return_sequences=True ,input_shape=(x,features))(oneMoreLayer)
    ...
    def giveAModelNum(self):
        ...
        i = self.layers
        oneMoreLayer=(BatchNormalization())(input1)
        oneMoreLayer = Dense(features*5 ,kernel_initializer='normal', activation='relu',kernel_regularizer=regularizers.l2(0.1))(oneMoreLayer)
    ...
    def trainMeSenpaiY(self,epochs):
        x = int(self.X.size / features)
        self.X=np.reshape(self.X,[1,x,features])
        out=self.Y[:, 1]
        out=out[:,newaxis]
        history = self.model.fit([self.X[:,:,:]], [out],
                                 epochs=epochs, batch_size=2048, verbose=1)
...
np.random.seed(42)
X = genfromtxt('trainingData_2Out37.csv', delimiter=',')
Y = genfromtxt('trainingData_2Out37.csv', delimiter=',')
X1=np.delete(X,0,0)
